...
client.on('message', async msg => {
  ...
  const regexp1 = /(["]|[`])(.*?)(["]|[`])/
  const text = (regexp1).test(content) ? regexp1.exec(content)[2] : ((/\s/g).test(content) ? content.split(/\s/g)[0] : content)
  ...
  const wikimd = word => {
    ...
    embed.addField(`[${link}] (${word})`, `${wiki(word)}`)
    return `*${word}[${link}]*`
  }
  ...
  if (short_description) embed.addField('Short description', short_description[0])
...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  const regexp1 = /(["]|[`])(.*?)(["]|[`])/
  const text = (regexp1).test(msg.content) ? regexp1.exec(msg.content)[2] : ((/\s/g).test(msg.content) ? msg.content.split(/\s/g)[0] : msg.content)
  let [ detections ] = await translate.detect(text)
  detections = Array.isArray(detections) ? detections : [detections]
  const lang = detections[0].language
  //const wiki = new Wikiapi(lang)
